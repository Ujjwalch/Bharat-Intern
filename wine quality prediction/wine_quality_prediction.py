...
#I will be using white wine dataset
...
#Let's look Correlation chart
colormap = plt.cm.viridis
plt.figure(figsize=(12,14))
plt.title('Correlation of Features', y=1.05, size=15)
sns.heatmap(data.astype(float).corr(),linewidths=0.1,vmax=1.0, square=True, 
            linecolor='white', annot=True)
#I will use Random Forest Classifier to train my prediction model
...
#here we will make a GUI using TKINTER
master = Tk()
master.title('Wine Quality Predictor')
...
#Here we will set how we want to take input
e1 = Entry(master)
e2 = Entry(master)
e3 = Entry(master)
e4 = Entry(master)
...
#here we will place them on gui canvas
...
#Now we will assign two buttons
#one will be to find quality while other will be to quit
Button(master, text='Quit', command=master.destroy,width=17).grid(row=11, column=0, sticky=W, pady=3)
